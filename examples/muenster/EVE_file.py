...
class EveInput(InputFromFolder):
    ...
    def _write_caen_pars(self, filename):
        ...
        with open(filename + ".caenpars", "w") as caenfile:
            ...
            caenfile.write("chan_active, board 0" + (8 * "\t %d") % tuple(self.file_caen_pars["chan_active"][0]) + "\n")
            caenfile.write(
                    "chan_active, board 1" + (8 * "\t %d") % tuple(self.file_caen_pars["chan_active"][1]) + "\n\n")
            caenfile.write("chan_dac, board 0" + (8 * "\t %d") % tuple(self.file_caen_pars["chan_dac"][0]) + "\n")
            caenfile.write("chan_dac, board 1" + (8 * "\t %d") % tuple(self.file_caen_pars["chan_dac"][1]) + "\n\n")
            caenfile.write("threshold, board 0" + (8 * "\t %d") % tuple(self.file_caen_pars["threshold"][0]) + "\n")
            caenfile.write("threshold, board 1" + (8 * "\t %d") % tuple(self.file_caen_pars["threshold"][1]) + "\n\n")
            caenfile.write(
                    "zs_threshold, board 0" + (8 * "\t %d") % tuple(self.file_caen_pars["zs_threshold"][0]) + "\n")
            caenfile.write(
                    "zs_threshold, board 1" + (8 * "\t %d") % tuple(self.file_caen_pars["zs_threshold"][1]) + "\n\n")
            caenfile.write("zero_offset, board 0" + (8 * "\t %d") % tuple(self.file_caen_pars["zero_offset"][0]) + "\n")
            caenfile.write(
                    "zero_offset, board 1" + (8 * "\t %d") % tuple(self.file_caen_pars["zero_offset"][1]) + "\n\n")
            caenfile.write("nof_active_channels \t %d\t%d" % (
                self.file_caen_pars["nof_active_channels"][0], self.file_caen_pars["nof_active_channels"][1]) + "\n")
