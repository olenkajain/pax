...
class S2LinearityCorrection(plugin.TransformPlugin):
    """Inside a peak, the peak area is a sum of all hit area.
    This introduce a bias due to zero length suppression.
    In this plugin, we consider to add all pulse values inside a peak to calculate the peak area,
    to test how much it affect the S2 linearity.
    """
    ...
    def transform_event(self, event):
        ...
        nPmts = 248
        nPmtsTop = 127
        s2_sorted = list(sorted([p for p in event.peaks if p.type == 's2' and p.detector == 'tpc'],
